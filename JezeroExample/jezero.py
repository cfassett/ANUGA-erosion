...
def main():
    try:
        try:
            ...
            domain.set_quantity('stage', -10000)
            stagepolygon=anuga.read_polygon('stage.csv')
            ...
            operatezero = suspension_erosion_operator(domain, poly_bound,grainD=grainD, gravity=anuga.g)
            operateone = shear_erosion_operator(domain,poly_bound,grainD=grainD, gravity=anuga.g)
            operatetwo = friction_operator(domain,poly_bound,grainD=grainD, gravity=anuga.g)
            operatethree = AoR_operator(domain, poly_bound)  #no grainsize or gravity dependence for the angle of repose change

            initialdomain=copy.deepcopy(domain)
            initial=domain.get_quantity('elevation').get_values(interpolation_points=[[45431.41238216,29844.44630651]], location='centroids')
            ystep=3600
            ftime=86400*100
            count=0
            volthresh=1000
            for t in domain.evolve(yieldstep=ystep, finaltime=ftime):
                ...
                print 'xmom:'+str(domain.get_quantity('xmomentum').get_values(interpolation_points=[[45431.41238216,29844.44630651]], location='centroids'))
                breacherosion=domain.get_quantity('elevation').get_values(interpolation_points=[[45431.41238216,29844.44630651]], location='centroids')-initial
